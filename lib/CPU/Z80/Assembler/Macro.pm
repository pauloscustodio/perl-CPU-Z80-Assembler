...
are HOP::Stream objects returning sequences of tokens as defined 
in CPU::Z80::Assembler::Lexer.
...
Takes a HOP::Stream object as input. The stream returns lexical tokens on each 
HOP::Stream::drop call, as defined in CPU::Z80::Assembler::Lexer.
...
the definition are replaced by the actual arguments, and the labels are renamed to a 
unique name to allow multiple expansions of the same macro.
